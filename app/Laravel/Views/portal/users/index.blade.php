...
<div class="card shadow mb-4 border-bottom-secondary">
    <div class="card-header py-3 d-sm-flex align-items-center justify-content-between">
        ...
        @if($auth->canAny(['portal.users.create'], 'portal'))
        <a class="btn btn-sm btn-primary" href="{{route('portal.users.create')}}">Create User</a>
        @endif
    </div>
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            @if($auth->canAny(['portal.users.view'], 'portal'))
                            <a href="{{route('portal.users.show', [$user->id])}}">{{str_pad($user->id, 5, "0", STR_PAD_LEFT)}}</a><br>
                            @else
                            <a href="#">{{str_pad($user->id, 5, "0", STR_PAD_LEFT)}}</a><br>
                            @endif
                            <div>{{$user->name}}</div>
                        </td>
                        ...
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu animated--fade-in" aria-labelledby="dropdownMenuButton" style="">
                                    ...
                                    <a class="dropdown-item" href="{{route('portal.users.edit_password', [$user->id])}}">Reset Password</a>
                                    @endif
                                    @if($auth->canAny(['portal.users.update_status'], 'portal'))
                                    <a class="dropdown-item btn-activation" data-url="{{route('portal.users.update_status', [$user->id])}}" data-status="{{$user->status}}" type="button">{{$user->status == 'active' ? 'Deactivate Account' : 'Activate Account'}}</a>
                                    @endif
                                </div>
                            </div>
                        </td>
